					...
					If ($value#"")
						...
						Case of 
							...
							: ($value="@Sous-formulaires-en-liste@")
								...
								//$value:=Replace string($value; "845377"; "4672435"; *)
							...
							: ($value="@Get-current-data-source@") && ($value#"@SQL-Get-current-data-source@")
								
								$value:=Replace string:C233($value; "Get-current-data-source"; "SQL-Get-current-data-source"; *)
								
							: ($value="@GET-DATA-SOURCE-LIST@") && ($value#"@SQL-GET-DATA-SOURCE-LIST@")
								
								$value:=Replace string:C233($value; "GET-DATA-SOURCE-LIST"; "SQL-GET-DATA-SOURCE-LIST"; *)
								
							: ($value="@-.200-6229469@")
								
								$value:=Replace string:C233($value; "-.200-6229469"; "Handling-tables.200-6229469"; *)
								
							: ($value="@4D-Write-Pro4D.200-6229459@")
								
								$value:=Replace string:C233($value; "4D-Write-Pro4D.200-6229459"; "Handling-tables.200-6229469"; *)
								
							: ($value="@IP-Settings.300-4672424@")
								
								$value:=Replace string:C233($value; "IP-Settings.300-4672424"; "IP-Settings.300-6330535"; *)
								
							: ($value="@WP-GET-ATTRIBUTES.301-4523013@")
								
								$value:=Replace string:C233($value; "WP-GET-ATTRIBUTES.301-4523013"; "WP-GET-ATTRIBUTES.301-6229519"; *)
									...
									Else 
										...
										Else 
											...
											Case of 
												...
												: ($value="@Monitor-Page@")
												: ($value="@Debugger@")
												: ($value="@dsprovideDataKey@")
												: ($value="@Constants-Page@")
												: ($value="@Break-List@")
												: ($value="@Specifying-the-action-of-a-menu@")
												: ($value="@Security-page@")
												: ($value="@General-page@")
												: ($value="@Interface-page@")
												: ($value="@Using-user-settings@")
												: ($value="@Constants@")
												: ($value="@SDI-mode-on-Windows@")
												: ($value="@dataClassquery@")
												: ($value="@collectioncopy@")
												: ($value="@dsencryptionStatus@")
												: ($value="@VP-Run-offscreen-area@")
												: ($value="@IF.@")
												: ($value="@VP-SET-ALLOWED-METHODS@")
												: ($value="@Project-method-references@")
												: ($value="@On-VP-Ready-form-event@")
												: ($value="@transporterhost@")
												: ($value="@transporterport@")
												: ($value="@transporterconnectionTimeOut@")
												: ($value="@transporteracceptUnsecureConnection@")
												: ($value="@transporterauthenticationMode@")
												: ($value="@transporteruser@")
												: ($value="@transporterlogFile@")
												: ($value="@transporter@")
												: ($value="@transporteur@")
												: ($value="@IMAP-transporteraddFlags@")
												: ($value="@IMAP-transporterappend@")
												: ($value="@IMAP-transportercopy@")
												: ($value="@IMAP-transporterdelete@")
												: ($value="@IMAP-transporterexpunge@")
												: ($value="@IMAP-transportergetBoxList@")
												: ($value="@IMAP-transportergetDelimiter@")
												: ($value="@IMAP-transportergetBoxInfo@")
												: ($value="@IMAP-transportergetMIMEAsBlob@")
												: ($value="@IMAP-transportermove@")
												: ($value="@IMAP-@")
												...
												: ($value="@Syntax-Error-Window@")
												: ($value="@RepeatUntil@")
												: ($value="@UtiliserFin-utiliser@")
												: ($value="@Commandes-du-theme-BLOB@")
												: ($value="@BoucleFin-de-boucle@")
												: ($value="@Parametrages-du-serveur-Web@")
												: ($value="@SMTP-@")
													...
													Else 
														...
														If (Match regex:C1019("(.+\\.)[^.]+(\\.[a-z]{2})\\.html"; $value; 1; $pos; $len))
															...
															If ($files.length#0)
																...
																$found:=True:C214
															Else 
																$files:=$file.parent.parent.folder("20.1").files().query("name == :1"; $command+"@"+$language)
																If ($files.length#0)
																	$targetFile:=$files[0]
																	$found:=True:C214
																Else 
																	$files:=$file.parent.parent.folder("20").files().query("name == :1"; $command+"@"+$language)
																	If ($files.length#0)
																		$targetFile:=$files[0]
																		$found:=True:C214
																	End if 
